interface StudyCandidatePin {
  readonly questionId: string
  readonly questionVersionId: string
}

interface RandomStudyCandidate extends StudyCandidatePin {
  readonly isRecent: boolean
}

interface BookmarkStudyCandidate extends StudyCandidatePin {
  readonly createdAt: Date
}

type DailyReviewCandidateStatus = 'NEW' | 'REVIEWING' | 'AGAIN' | 'SOLVED'

interface DailyReviewStudyCandidate extends StudyCandidatePin {
  readonly nextReviewAt: Date
  readonly status: DailyReviewCandidateStatus
}

interface WrongNoteStudyCandidate extends StudyCandidatePin {
  readonly lastWrongAt: Date
  readonly wrongCount: number
}

interface WeaknessStudyCandidate extends StudyCandidatePin {
...
const assertRequestedCount = (requestedCount: number): void => {
  if (!Number.isSafeInteger(requestedCount) || requestedCount < 1) {
    throw new Error('requestedCount는 1 이상의 safe integer여야 합니다.')
  }
}
...
const assertUniqueCandidates = <Candidate extends StudyCandidatePin>(
  ...
  for (const candidate of candidates) {
    if (
      ...
      throw new Error('candidate ID는 비어 있을 수 없습니다.')
    }
...
const shuffleCandidates = <Candidate extends StudyCandidatePin>(
  ...
  for (let index = shuffled.length - 1; index > 0; index -= 1) {
    ...
    if (!Number.isFinite(randomValue) || randomValue < 0 || randomValue >= 1) {
      throw new Error('RNG는 0 이상 1 미만의 유한수를 반환해야 합니다.')
    }
    const swapIndex = Math.floor(randomValue * (index + 1))
    const current = shuffled[index]
    const selected = shuffled[swapIndex]
    if (!current || !selected) {
      throw new Error('candidate shuffle index가 유효하지 않습니다.')
    }
